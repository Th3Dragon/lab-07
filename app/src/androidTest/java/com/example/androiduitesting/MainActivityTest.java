...
public class MainActivityTest {
    ...
    public void testActivitySwitch() {
        ...
        // Click on the item at position zero in the Adapter view
        onData(is(instanceOf(String.class)))
                .inAdapterView(withId(R.id.city_list))
                .atPosition(0)
                .perform(click());

        // Check to see if certain UI elements are there
        onView(withId(R.id.city_name_view)).check(matches(isDisplayed()));
    ...
    public void testCityNameConsistency() {
        ...
        // Click on the newly added city (Edmonton)
        onView(withText("Edmonton")).perform(click());

        // Check to see if the displayed city name (in the second activity) matches the one pressed
    ...
    public void testBackButton() {
        ...
        // Check to see if certain UI elements are there (specific to ShowActivity)
        onView(withId(R.id.city_name_view)).check(matches(isDisplayed()));
        onView(withId(R.id.button_back)).check(matches(isDisplayed()));

        // Click on the back button to get back to MainActivity
        onView(withId(R.id.button_back)).perform(click());

        // Check to see if the certain UI elements are there (specific to MainActivity)
